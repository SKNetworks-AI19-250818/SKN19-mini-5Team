...
| 박&nbsp;도&nbsp;연 | 배웠던 여러 모델을 이용해 실데이터를 학습시키고 성능을 평가해보았다. 그리고 간단하게라도 이용자에게 값을 직접 받아 피쳐 데이터로 변환해 예측을 수행하는 페이지를 구현했는데, 생각보다 많은 처리를 필요로 했다. 성능 높이기와 동시에 이용자에게 어느 정도의 값을 받고 나머지 피쳐는 어떤 식으로 처리해야 유용한 서비스가 될 수 있는지 고려하는 게 다음 프로젝트의 숙제일 것 같다. | 
