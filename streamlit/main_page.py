...
# 특이사항 + 문제사항
# 1. property_type은 room_type에 따라 가장 많이 있는 칼럼값으로 자동 지정했습니다.
# 2. 여러 어매니티 칼럼 중 feature importance가 높은 것 위주로 열두 개만 추가 옵션 선택 가능하도록 구현했습니다.
# 3. 어매니티 칼럼의 기본값은 최빈값으로 설정했습니다.
# 4. days_since_first_review 컬럼값이 min: -3, max: 5028의 컬럼이라 기존 first_review 칼럼값처럼 날짜 형식으로 받았습니다.
# 5. model_LGBMReg_r2_test_0.71.joblib은 범섭님이 디스코드에 올려주신 assets.zip파일 압축 해제 후 streamlit/assets로 옮겨 사용했습니다.
#    따라서 `streamlit run main_page.py`는 streamlit 폴더에서 실행해야 모델이 잘 불러와집니다
# 6. 여러 피쳐를 직접 받지만 몇몇 피쳐는 에어비앤비에서 한눈에 확인이 불가한 문제 있습니다.
#    (ex.room_type 지정 애매하고, 숙소 운영 기간 파악 어렵고, 추가 옵션 여러 개를 선택하면 에어비앤비 사이트에서도 똑같이 눌러줘야 합니다...)
# 7. 
...
if not st.session_state.start_clicked:
    ...
    if len(selected_options)>0:
        with col3:
            ...
            if st.button("예측 결과 보기", key='first_view'):
                st.session_state.show_result = True
     

    # 추가 옵션 선택 버튼을 눌렀을 때
    if st.session_state.extra_input and not st.session_state.show_result:
        # 어매니티 칼럼 저장
        ...
        st.markdown("---")
        if st.button("예측 결과 보기", key='second_view'):
            get_X = make_X(neighbourhood, room_type, accommodates,
                           bedrooms, beds, bathrooms, review_scores_rating,
                           amnt_values.values(), first_date)
    ...
    # 예측 결과 보기 버튼을 눌렀을 때
        ...
        # 어매니티 칼럼 저장
        amnt_values = amnt_default_value_dict
        for k in selected_options:
            amnt_values[k] = 1
